 ...
 *
 * Usage:
 *   node backfill-comic-word-explanations.js --dry-run    # Preview what will be done
 *   node backfill-comic-word-explanations.js              # Actually generate word explanations
 *   node backfill-comic-word-explanations.js --episode 13 # Backfill specific episode only
 */
...
const specificEpisode = args.find(arg => arg.startsWith('--episode'))
  ? parseInt(args[args.indexOf('--episode') + 1])
  : null;

// We'll call the precompute function by requiring the compiled version
// The function is in src/lib/ai/precompute/wordPrecompute.ts but we need the compiled JS
let precomputeWordExplanationsFunc = null;

async function initPrecomputeFunction() {
  try {
    // Try to require from the Next.js build
    const wordPrecompute = require('./src/lib/ai/precompute/wordPrecompute.ts');
    precomputeWordExplanationsFunc = wordPrecompute.precomputeWordExplanations;
  } catch (err) {
    console.error('⚠️  Could not load precomputeWordExplanations function');
    console.error('   Make sure you have built the Next.js app: npm run build');
    throw new Error('Function not available - run npm run build first');
  }
}

async function extractComicText(comicData) {
...
async function generateWordExplanationsForEpisode(comic, comicId, episodeNumber) {
  ...
  const comicText = await extractComicText(comic);
  ...
  try {
    ...
    // Use the same precomputeWordExplanations function as the scheduler
    const { precomputeWordExplanations } = require('./src/lib/ai/precompute/wordPrecompute.ts');
...
async function backfillComicWordExplanations() {
  ...
  for (const doc of comicsSnapshot.docs) {
    ...
    const result = await generateWordExplanationsForEpisode(comic, doc.id, episodeNumber);
  ...
  if (isDryRun && results.dryRun > 0) {
    ...
    console.log('   node backfill-comic-word-explanations.js');
