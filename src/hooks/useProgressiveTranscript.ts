...
const mapToSegments = (segments: any[]): TranscriptSegment[] =>
  (segments || []).map((segment, index) => ({
    ...
    endTime: typeof segment.endTime === 'number' ? segment.endTime : segment.end ?? segment.start ?? 0,
    words: segment.words,
    ...
    const fetchRawTranscript = async () => {
      try {
        ...
        const response = await fetch(
          `/api/youtube/extract?videoId=${videoId}&phase=raw`,
          {
            method: 'POST',
        ...
        try {
          const aiResponse = await fetch(
            `/api/youtube/extract?videoId=${videoId}&phase=enhance`,
            {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
