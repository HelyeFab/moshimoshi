...
export default function LearnContent() {
  ...
  const handleRound2Complete = async (results: any) => {
    ...
    // NOTE: Streak is automatically updated via XP tracking
    if (user) {
  ...
  const handleSessionComplete = async () => {
    try {
      ...
      // Calculate XP and record session (includes auto-streak update if XP >= 10)
      const xp = progressManager.calculateSessionXP(updatedSessionState, session.sessionStats)

      if (user) {
        // Award XP (non-blocking)
        addXP(xp, 'kanji_mastery').catch(err => {
        ...
        try {
          await recordSession({
            type: 'kanji_mastery',
            itemsReviewed: session.kanji.length,
            accuracy: session.sessionStats.averageAccuracy * 100,
            duration: session.sessionStats.timeSpentSeconds * 1000, // Convert to ms
            xpEarned: xp  // Pass XP to determine streak update
          })
        } catch (err) {
          console.error('Failed to record kanji session:', err)
        }
      }
