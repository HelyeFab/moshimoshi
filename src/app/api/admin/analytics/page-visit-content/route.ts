...
export const GET = withAdminAuth(async (request: NextRequest) => {
  try {
    ...
    const aggregate = new Map<string, { path: string; totalViews: number; lastViewAt: string | null }>()
    ...
    snapshot.docs.forEach((doc) => {
      ...
      if (!existing) {
        aggregate.set(path, { path, totalViews, lastViewAt })
      } else {
    ...
    const items = await Promise.all(
      sorted.map(async (item) => {
        ...
        try {
          ...
          console.warn('[page-visit-content] Failed to resolve display name:', error)
        }
    ...
    console.error('[API /admin/analytics/page-visit-content] Error:', error)
    return NextResponse.json(
      { error: { message: 'Failed to fetch page visit content summary' } },
      { status: 500 }
    )
  }
})
