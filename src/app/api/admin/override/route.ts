...
import { setOverride, removeOverride, getUserOverrides } from '@/lib/entitlements/adminEvaluator'
...
export const POST = withAdminAuth(async (request: NextRequest, context: AdminContext) => {
  try {
    ...
    if (!override || typeof override !== 'object') {
      return NextResponse.json({ error: 'Invalid override object' }, { status: 400 })
    }
    ...
    if ('note' in override) {
      if (override.note !== null && typeof override.note !== 'string') {
        return NextResponse.json({ error: 'Invalid note: must be string or null' }, { status: 400 })
      }
    ...
    await setOverride(userId, featureId as FeatureId, {
...
export const DELETE = withAdminAuth(async (request: NextRequest, context: AdminContext) => {
  try {
    ...
    // Remove the override
    await removeOverride(userId, featureId as FeatureId, context.user.uid)
