...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [nowOnAirResponse, todayScheduleResponse, educationalNowResponse] = await Promise.all([
      ...
      fetch(
        `http://api.nhk.or.jp/v2/pg/list/${TOKYO_AREA}/${service}/${dateStr}.json?key=${nhkApiKey}`,
        {
          headers: { 'User-Agent': 'Moshimoshi-Demo/1.0' },
        }
      ),
      ...
      service === NHK_SERVICES.GENERAL
        ? fetch(
            `http://api.nhk.or.jp/v2/pg/now/${TOKYO_AREA}/${NHK_SERVICES.EDUCATIONAL}.json?key=${nhkApiKey}`,
            {
              headers: { 'User-Agent': 'Moshimoshi-Demo/1.0' },
            }
          )
    ...
    if (educationalNow) {
      const currentEducational = enhancePrograms(
        extractCurrentPrograms(educationalNow, NHK_SERVICES.EDUCATIONAL)
      )
 ...
 */
function filterNewsProgramsFromSchedule(
...
function filterEducationalPrograms(schedule: NHKProgramListResponse, serviceId: string): any[] {
  ...
  return programs.filter(program => {
    const educationalGenres: string[] = [
      NHK_GENRES.EDUCATION,
      NHK_GENRES.LANGUAGE,
      NHK_GENRES.CHILDREN,
    ]
    const isEducational = program.genres.some((genre: string) => educationalGenres.includes(genre))
...
function generateDemoStats(
  ...
  const newsPrograms = generalPrograms.filter(
    p => p.genres.includes(NHK_GENRES.NEWS) || /ニュース/i.test(p.title)
  )
  const educationalPrograms = generalPrograms.filter(
    p => p.genres.includes(NHK_GENRES.EDUCATION) || /講座|学習/i.test(p.title)
  )
