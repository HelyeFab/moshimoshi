...
export default function SignUpPage() {
  ...
  const { isAuthenticated, loading: authLoading, signOut: signOutUser, user } = useAuth()
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (password !== confirmPassword) {
      setError(strings.auth.signup.errors.passwordMismatch)
      setLoading(false)
      return
    }
    ...
    try {
      ...
      const response = await fetch('/api/auth/signup', {
        ...
        body: JSON.stringify({ email, password, displayName, termsAccepted, recaptchaToken })
      })
  ...
  const handleAppleSignIn = async () => {
    logger.auth('Apple sign up clicked')
    setLoading(true)
    setError('')
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background-light to-japanese-mizu/20 dark:from-dark-850 dark:to-dark-900 flex flex-col overflow-y-auto">
      <div className="flex-1 flex sm:items-center justify-center px-4 py-8 sm:py-12">
        ...
        <div className="bg-white dark:bg-surface-dark rounded-2xl shadow-xl p-8">
          ...
          <div className="relative">
            ...
            >
              ...
              {strings.auth.signup.alternativeAuth.appleButton}
            </button>
          ...
          <form onSubmit={handleSignUp} className="space-y-4">
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                {strings.auth.signup.form.labels.confirmPassword}
              </label>
              <div className="relative">
                <input
                  id="confirmPassword"
                  ...
                  className="w-full px-4 py-2 pr-11 border border-gray-300 dark:border-dark-600 rounded-lg bg-white dark:bg-dark-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                  placeholder={strings.auth.signup.form.placeholders.confirmPassword}
                  required
                />
                <button
                  type="button"
          ...
          {MAGIC_LINK_ENABLED && !showMagicLink && !magicLinkSent && (
            <div className="relative">
              <button
                type="button"
