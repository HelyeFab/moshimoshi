...
export default function TestFlashcardsPage() {
  ...
  const createBaseDeck = (uid: string): FlashcardDeck => {
    const now = Date.now();
    return {
  ...
  const addCardToDeck = (deck: FlashcardDeck, front: string, back: string): FlashcardDeck => {
    const now = Date.now();
    ...
    const cards = [...deck.cards, newCard];
    return {
      ...deck,
      cards,
      stats: {
