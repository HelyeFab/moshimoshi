  ...
  const fetchCodeData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Show notification if auto-regeneration occurred
        if (data.autoRegenerated) {
          showToast('Types were auto-regenerated to match the JSON config.', 'success', 4000)
        } else if (data.regenerationError) {
          showToast(`Auto-regeneration failed: ${data.regenerationError}`, 'error', 5000)
        }
      } else {
  ...
  const handleGenerateTypes = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast(`Failed to generate types: ${result.error || 'Unknown error'}`, 'error', 6000)
  ...
  return (
    <div className="space-y-6">
      ...
      <StatCardGrid columns={4}>
        ...
        <StatCard label="Total Usage" value={totalUsage} icon="📊" />
        <StatCard label="Denied Requests" value={totalDenied} icon="🚫" color="red" />
        <StatCard
          label="Success Rate"
      ...
      {stats.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-4 lg:gap-6">
          {stats.map(stat => (
            <div
              key={stat.feature}
              className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 sm:p-4 lg:p-6"
            >
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs sm:text-sm font-medium">
                    {stat.averageUsagePerUser.toFixed(1)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
