...
export default function SettingsPage() {
  ...
  const loadUserPreferences = async () => {
    try {
      ...
      // NOTE: Don't call setLanguage here - the URL locale is the source of truth.
      // Calling setLanguage would navigate away and cause a redirect loop.
      // The language is already set correctly from the URL via I18nContext.
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="space-y-6">
          ...
          <div className="text-center mt-4">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {!user ? (
                <>🔓 {strings.settings?.storageTier?.guest || "Guest Mode - Settings won't be saved"}</>
              ) : isPremium ? (
                <>⭐ {strings.settings?.storageTier?.premium || "Premium - Settings sync across all devices"}</>
              ) : (
                <>💾 {strings.settings?.storageTier?.free || "Free Account - Settings saved locally"}</>
              )}
            </p>
          </div>
