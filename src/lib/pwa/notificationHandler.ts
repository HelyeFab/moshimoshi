  ...
  private async showNotification(title: string, options: NotificationOptions): Promise<void> {
    ...
    try {
      ...
      await registration.showNotification(title, {
        ...
        // timestamp is valid for ServiceWorker notifications but not in DOM NotificationOptions
  ...
  private async restoreScheduledNotifications(): Promise<void> {
    ...
    for (const schedule of schedules) {
      ...
      if (scheduledTime <= now) {
        ...
        this.scheduleNotification(schedule.title, schedule.body, delay, schedule.data)
      }
    }
  }
