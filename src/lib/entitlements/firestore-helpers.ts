...
import { UsageBucket, EntitlementLog, FeatureId, PlanType } from '@/types/entitlements'
 ...
 */
export async function getUsageBucket(userId: string, date: string): Promise<UsageBucket> {
  const db = getDb()
 ...
 * Get usage for today
 */
export async function getTodayUsage(userId: string): Promise<Partial<Record<FeatureId, number>>> {
 ...
 */
export async function logDecision(decision: EntitlementLog): Promise<void> {
  const db = getDb()
  await db
    .collection('logs')
    .doc('entitlements')
...
export async function cleanupOldUsageBuckets(
  ...
  const db = getDb()
  const bucketsRef = db.collection('users').doc(userId).collection('usage')

  const oldBuckets = await bucketsRef.where('date', '<', cutoffString).get()
 ...
 */
export async function getUsageHistory(userId: string, days: number = 7): Promise<UsageBucket[]> {
  const db = getDb()
  const bucketsRef = db.collection('users').doc(userId).collection('usage')

  const snapshot = await bucketsRef.orderBy('date', 'desc').limit(days).get()
...
export async function getDecisionLogs(
  ...
  const db = getDb()
  const logsRef = db.collection('logs').doc('entitlements').collection('decisions')
...
export async function cleanupExpiredIdempotency(): Promise<number> {
  ...
  const expiredDocs = await db.collection('idempotency').where('expiresAt', '<', now).get()
 ...
 */
export async function resetFeatureUsage(userId: string, featureId: FeatureId): Promise<void> {
  const db = getDb()
  const today = getTodayBucket(new Date().toISOString())
