...
import {
  ...
  KanjiMnemonic,
  KanjiMnemonicComponent,
  AIServiceError
} from '../types';

// Internal type for parsed AI response
interface ParsedMnemonicResponse {
  mnemonic: string;
  meaning?: string;
  components?: KanjiMnemonicComponent[];
}

export class KanjiMnemonicProcessor extends BaseProcessor<KanjiMnemonicRequest, KanjiMnemonic> {
   ...
   * Validate the request
   */
  validateRequest(request: KanjiMnemonicRequest): void {
    if (!request.kanji) {
      throw new AIServiceError(
        'Kanji character is required',
        'VALIDATION_ERROR',
        400
      );
    }
    ...
    if (typeof request.kanji !== 'string' || request.kanji.trim().length === 0) {
      throw new AIServiceError(
        'Kanji must be a non-empty string',
        'VALIDATION_ERROR',
        400
      );
    }
    ...
    if (request.kanji.length > 4) {
      throw new AIServiceError(
        'Please provide a single kanji character',
        'VALIDATION_ERROR',
        400
      );
    }
...
- The mnemonic should work even without knowing Japanese`;
   ...
   */
  getUserPrompt(request: KanjiMnemonicRequest, config?: TaskConfig): string {
   ...
   */
  parseResponse(response: string): KanjiMnemonic {
    const parsed = this.parseJSON<ParsedMnemonicResponse>(response);

    // Validate required fields
    if (!parsed.mnemonic) {
      throw new AIServiceError(
        'Missing mnemonic in response',
