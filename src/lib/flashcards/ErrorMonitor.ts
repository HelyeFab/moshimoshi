...
export class ErrorMonitor {
  ...
  private async attemptAutoRecovery(error: ErrorEntry): Promise<void> {
    const pattern = this.errorPatterns.find(
      p => p.pattern.test(error.error.message) && p.autoRecover
    )
  ...
  getErrorsByType(type: ErrorEntry['error']['type']): ErrorEntry[] {
    return Array.from(this.errors.values()).filter(error => error.error.type === type)
  }
...
## Top Errors
${summary.topErrors.map((e: { message: string; count: number }, i: number) => `${i + 1}. ${e.message} (${e.count} occurrences)`).join('\n')}
