      ...
      if (stats) {
        ...
        if (thresholdKey) {
          ...
          if (stats.p95 > threshold * 2) {
            report.warnings.push(`${name} p95 (${stats.p95.toFixed(2)}ms) is very high`)
          }
        }
      }
    }
  ...
  private generateSuggestions(report: PerformanceReport): void {
    ...
    if (syncOps.some(([_, stats]) => stats.p95 > 5000)) {
      report.suggestions.push(
        'Sync operations are taking too long. Consider batching or queue optimization'
      )
    }
    ...
    if ('memory' in performance && (performance as any).memory) {
      ...
      if (usedMB > 100) {
        report.suggestions.push(
          `High memory usage detected (${usedMB.toFixed(2)}MB). Consider cleanup`
        )
      }
    }
