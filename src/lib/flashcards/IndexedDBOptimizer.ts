...
export class IndexedDBOptimizer {
  ...
  async initDB(): Promise<IDBPDatabase<OptimizedFlashcardDB>> {
    ...
    this.db = await openDB<OptimizedFlashcardDB>('OptimizedFlashcardDB', 2, {
      upgrade(db, oldVersion, _newVersion, transaction) {
        ...
        if (oldVersion === 1) {
          // Add cardCount index using the upgrade transaction
          if (db.objectStoreNames.contains('decks')) {
            const decksStore = transaction.objectStore('decks')
            if (!decksStore.indexNames.contains('cardCount')) {
              decksStore.createIndex('cardCount', 'stats.totalCards')
            }
          }
        }
  ...
  async bulkUpdateDecks(
    updates: Array<{ id: string; changes: Partial<FlashcardDeck> }>
  ): Promise<void> {
    const db = await this.initDB()
    const tx = db.transaction('decks', 'readwrite')
  ...
  async searchDecks(
    ...
    const { searchIn = ['name', 'description'], limit = 20 } = options
  ...
  private startCacheCleanup(): void {
    setInterval(async () => {
      ...
      if (this.db) {
        ...
        await Promise.all(oldEntries.map(key => tx.objectStore('deckCache').delete(key)))
        ...
        await Promise.all(oldQueries.map(key => tx.objectStore('queryCache').delete(key)))
