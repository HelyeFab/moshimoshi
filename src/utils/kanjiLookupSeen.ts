const STORAGE_PREFIX = 'kanji_lookup_seen_'
...
export const hasSeenKanjiLookup = (kanji: string): boolean => {
  const normalized = kanji.trim()
  if (!normalized) return false
  return loadSeenSet().has(normalized)
}

export const markKanjiLookupSeen = (kanji: string): void => {
  const normalized = kanji.trim()
