...
export default function KanaStudyMode({
  ...
  useEffect(() => {
    if (currentIndex === totalCharacters && !hasRecordedActivity && user?.uid) {
      try {
        ...
        const activities = JSON.parse(localStorage.getItem(`activities_${user.uid}`) || '{}')
  ...
  const handleSkip = async () => {
    ...
    if (user && user.uid) {
      const script = displayScript as 'hiragana' | 'katakana'
      await kanaProgressManagerV2.trackCharacterSkipped?.(
        script,
        character.id,
        user,
        isPremium ?? false
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-sm text-gray-600 dark:text-gray-400">
            {progress?.status === 'learned'
              ? t('kana.progress.learned')
              : progress?.status === 'learning'
                ? t('kana.progress.learning')
          ...
          <AnimatePresence mode="wait">
            {!isFlipped ? (
              ...
              >
                ...
                <AudioButton position="bottom-right" size="sm" onPlay={handlePlayAudio} />
                ...
                {character.pronunciation && (
                  <div
                    className="absolute bottom-16 left-4 right-4 bg-yellow-100 dark:bg-yellow-900/30
              ...
              {getExampleWords().length > 0 ? (
                getExampleWords().map((example, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-sm">
                    <span className="font-japanese text-lg text-gray-800 dark:text-gray-200">
                      {example.word}
                    </span>
