...
function getLeechSeverity(content: ReviewableContent | ReviewableContentWithSRS, threshold = 8): 'none' | 'warning' | 'critical' {
...
export default function ReviewCard({
  ...
  const renderContent = () => {
    ...
    switch (content.contentType) {
      case 'kana': return <KanaCard {...cardProps} />
      case 'kanji': return <KanjiCard {...cardProps} />
      case 'vocabulary': return <VocabularyCard {...cardProps} />
      case 'sentence': return <SentenceCard {...cardProps} />
      case 'grammar': return <CustomCard {...cardProps} />
      default: return <CustomCard {...cardProps} />
    }
  }

  return (
    <motion.div
      className="relative bg-soft-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 min-h-[400px] flex flex-col justify-center"
      initial={{ scale: 0.95 }}
      animate={{ scale: 1 }}
