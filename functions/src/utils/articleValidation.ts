 ...
 */
export function checkDuplicates(articles: ArticleData[]): ArticleData[] {
  const seen = new Map<string, ArticleData>();
 ...
 */
export function estimateDifficulty(content: string): string {
  ...
  if (kanjiRatio < 0.1) return 'N5'; // Very few kanji
  if (kanjiRatio < 0.2) return 'N4'; // Some kanji
  if (kanjiRatio < 0.3) return 'N3'; // Moderate kanji
  if (kanjiRatio < 0.4) return 'N2'; // Many kanji
 ...
 */
export function estimateReadingTime(content: string, difficulty: string): number {
  ...
  const speed = charsPerMinute[difficulty as keyof typeof charsPerMinute] || 300;
 ...
 */
export function extractKeyVocabulary(content: string, limit: number = 10): string[] {
