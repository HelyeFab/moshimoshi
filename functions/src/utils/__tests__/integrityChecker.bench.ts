...
import { IntegrityMockFactory, createTestScenario } from './test-utils/integrityChecker-mocks'
...
interface BenchmarkResult {
  ...
  p95Time: number
  passed: boolean
  threshold: { target: number; critical: number }
}
...
class PerformanceBenchmark {
  ...
  printReport(): void {
    ...
    for (const result of this.results) {
      ...
      console.log(
        `   Target: <${result.threshold.target}ms, Critical: <${result.threshold.critical}ms`
      )
    }
    ...
    for (const result of this.memoryResults) {
      ...
      console.log(
        `   Target: <${result.threshold.target}MB, Critical: <${result.threshold.critical}MB`
      )
    }
  ...
  getResults(): { timing: BenchmarkResult[]; memory: MemoryBenchmarkResult[] } {
...
describe('Integrity Checker Performance Benchmarks', () => {
  ...
  describe('Data Creation Performance', () => {
    it('should create 100 articles efficiently', async () => {
      const result = await benchmark.runBenchmark(
        'Create 100 Articles',
        () => {
          IntegrityMockFactory.createBulkNewsArticles(100)
        },
        50,
        { target: 50, critical: 100 }
      )
    ...
    it('should create 500 articles efficiently', async () => {
      const result = await benchmark.runBenchmark(
        'Create 500 Articles',
        () => {
          IntegrityMockFactory.createBulkNewsArticles(500)
        },
        20,
        { target: 200, critical: 500 }
      )
    ...
    it('should create missing content scenario efficiently', async () => {
      const result = await benchmark.runBenchmark(
        'Create Missing Content Scenario (100 items)',
        () => {
          IntegrityMockFactory.createMissingContentScenario(100)
        },
        30,
        { target: 100, critical: 250 }
      )
  ...
  describe('Stats Calculation Performance', () => {
    it('should calculate stats from 50 logs efficiently', async () => {
      const logs = IntegrityMockFactory.createBulkIntegrityLogs(50)

      const result = await benchmark.runBenchmark(
        'Calculate Stats (50 logs)',
        () => {
  ...
  describe('Queue Generation Performance', () => {
    it('should generate repair queue from 100 items efficiently', async () => {
      const scenario = createTestScenario.contentNeedingRepair()

      const result = await benchmark.runBenchmark(
        'Generate Repair Queue (100 items)',
        () => {
    ...
    it('should handle high volume scenario (500 items) efficiently', async () => {
      const scenario = createTestScenario.highVolumeLoad()

      const result = await benchmark.runBenchmark(
        'Process High Volume (500 articles, 100 stories)',
        () => {
  ...
  describe('Memory Usage', () => {
    it('should maintain acceptable memory usage with 500 articles', () => {
      ...
      const articles = IntegrityMockFactory.createBulkNewsArticles(500)
      const stories = IntegrityMockFactory.createBulkStories(100)

      const result = benchmark.measureMemory(
        '500 Articles + 100 Stories',
        MEMORY_THRESHOLDS.ARTICLES_500
      )
    ...
    it('should maintain acceptable memory usage with 1000 articles', () => {
      ...
      const articles = IntegrityMockFactory.createBulkNewsArticles(1000)
      const stories = IntegrityMockFactory.createBulkStories(200)
      const logs = IntegrityMockFactory.createBulkIntegrityLogs(100)

      const result = benchmark.measureMemory(
        '1000 Articles + 200 Stories + 100 Logs',
        MEMORY_THRESHOLDS.ARTICLES_1000
      )
  ...
  describe('Concurrent Operations', () => {
    it('should handle parallel ID generation efficiently', async () => {
      const result = await benchmark.runBenchmark(
        'Parallel ID Generation (100 concurrent)',
        () => {
          for (let i = 0; i < 100; i++) {
            IntegrityMockFactory.generateId('parallel')
          }
        },
        50,
        { target: 10, critical: 50 }
      )
    ...
    it('should handle parallel log creation efficiently', async () => {
      const result = await benchmark.runBenchmark(
        'Parallel Log Creation (50 concurrent)',
        () => {
          for (let i = 0; i < 50; i++) {
            IntegrityMockFactory.createIntegrityLog()
          }
        },
        30,
        { target: 50, critical: 150 }
      )
  ...
  describe('Batch Processing', () => {
    it('should process batches of 30 items efficiently', async () => {
      const articles = IntegrityMockFactory.createBulkNewsArticles(30)

      const result = await benchmark.runBenchmark(
        'Process Batch (30 items)',
        () => {
          ...
          const filtered = results.filter(
            r => r.needsAudio || r.needsTranslation || r.needsWordExplanations
          )
    ...
    it('should chunk large datasets efficiently', async () => {
      const articles = IntegrityMockFactory.createBulkNewsArticles(300)
      const chunkSize = 30

      const result = await benchmark.runBenchmark(
        'Chunk 300 items into batches of 30',
        () => {
          const chunks = []
...
function calculatePriority(
  article: ReturnType<typeof IntegrityMockFactory.createNewsArticle>
): number {
  let priority = 0

  // Audio is highest priority
  if (!article.hasAudio) priority += 100

  // Translations are medium priority
  if (!article.hasTranslation) priority += 50

  // Word explanations are lower priority
  if (!article.hasWordExplanations) priority += 25
  ...
  if (daysSincePublished < 1) priority += 50
  else if (daysSincePublished < 3) priority += 25
