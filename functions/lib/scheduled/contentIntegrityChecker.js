...
exports.contentIntegrityCheckerFunction = (0, scheduler_1.onSchedule)({
    ...
    try {
        ...
        await (0, integrityIdempotency_1.markCheckStarted)(checkId, 'scheduled', 'scheduler', scheduleTime);
        const result = await (0, integrityChecker_1.runIntegrityCheck)(adminKey, checkId);
        // Log results to Firestore
        await db.collection('integrity_check_logs').add(Object.assign(Object.assign({}, result), { type: 'scheduled', checkId, createdAt: admin.firestore.FieldValue.serverTimestamp() }));
        ...
        logger.info('[ContentIntegrityChecker] Check complete', {
            ...
            newsRepaired: result.newsArticles.repaired.audio +
        ...
        const totalMissing = result.newsArticles.missingAudio.length +
        ...
        await (0, integrityIdempotency_1.markCheckCompleted)(checkId, 'failed', error instanceof Error ? error.message : 'Unknown error');
...
exports.manualIntegrityCheckerFunction = (0, https_1.onCall)({
    ...
    if (!request.auth && adminKey !== expectedAdminKey) {
        throw new https_1.HttpsError('unauthenticated', 'User must be authenticated or provide valid admin key to run integrity check');
    }
    ...
    const triggeredBy = ((_b = request.auth) === null || _b === void 0 ? void 0 : _b.uid) || 'admin-key';
    ...
    const checkId = (0, integrityIdempotency_1.generateCheckId)('manual', timeSlot, triggeredBy);
    const instanceId = `manual_${checkId}`;
    // Extract options from request data (for manual triggers)
    const options = {};
    if (((_c = request.data) === null || _c === void 0 ? void 0 : _c.maxStories) !== undefined) {
        options.maxStories = Math.min(5, Math.max(1, parseInt(request.data.maxStories) || 1));
    }
    if (((_d = request.data) === null || _d === void 0 ? void 0 : _d.maxArticles) !== undefined) {
    ...
    if (await (0, integrityIdempotency_1.wasCheckProcessed)(checkId)) {
        ...
        throw new https_1.HttpsError('already-exists', `Integrity check already ran at ${timeSlot}. Please wait a minute before triggering again.`);
    }
    // Try to acquire lock (manual can fail if another check is running)
    const lockAcquired = await (0, integrityIdempotency_1.tryAcquireLock)(instanceId);
    if (!lockAcquired) {
        throw new https_1.HttpsError('already-exists', 'Another integrity check is currently running. Please try again later.');
    }
    try {
        await (0, integrityIdempotency_1.markCheckStarted)(checkId, 'manual', triggeredBy);
        const result = await (0, integrityChecker_1.runIntegrityCheck)(expectedAdminKey, checkId, options);
        // Log results to Firestore
        await db.collection('integrity_check_logs').add(Object.assign(Object.assign({}, result), { type: 'manual', checkId,
            triggeredBy, createdAt: admin.firestore.FieldValue.serverTimestamp() }));
        await (0, integrityIdempotency_1.markCheckCompleted)(checkId, 'completed');
        ...
        throw new https_1.HttpsError('internal', error instanceof Error ? error.message : 'Integrity check failed');
    }
    finally {
        await (0, integrityIdempotency_1.releaseLock)(instanceId);
    }
