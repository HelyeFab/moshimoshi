  ...
  try {
    ...
    try {
      await db
        ...
        .add(
          Object.assign(Object.assign({}, summary), {
            type: 'scheduled',
            createdAt: admin.firestore.FieldValue.serverTimestamp(),
          })
        )
    ...
    if (summary.totalArticles > 0) {
      ...
      try {
        ...
        if (!articlesSnapshot.empty) {
          ...
          try {
            const translationResults = await (0,
            translationPreGenerator_1.generateBatchTranslations)(articles)
          ...
          try {
            ...
            const wordExplanationResults = await (0,
            wordExplanationPreGenerator_1.generateBatchWordExplanations)(articlesWithWords)
...
async function runPreCachingPipeline(articleIds) {
  ...
  logger.info(
    '[PreCache] Pipeline completed',
    Object.assign(Object.assign({}, result), { totalCost: result.totalCost.toFixed(4) })
  )
  return result
}
