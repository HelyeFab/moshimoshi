...
class TranscriptProcessor extends BaseProcessor_1.BaseProcessor {
     ...
     */
    async processInBatches(request, config, processingType, batchSize) {
        var _a;
        const segments = request.content.transcript;
        const batches = [];
        // Split into batches
        ...
        const batchConfig = Object.assign(Object.assign({}, config), { timeout: 60000 // Increase timeout to 60 seconds for batch processing
         });
        for (let i = 0; i < batches.length; i++) {
            console.log(`  Processing batch ${i + 1}/${batches.length} (${batches[i].length} segments)...`);
            const batchRequest = Object.assign(Object.assign({}, request), { content: Object.assign(Object.assign({}, request.content), { transcript: batches[i] }) });
            try {
        ...
        if (allSegments.length === 0) {
            throw new types_1.AIServiceError('All batches failed to process', 'BATCH_PROCESSING_FAILED', 500);
        }
        console.log(`✅ Successfully processed ${allSegments.length} segments across ${batches.length} batches`);
        return {
            data: {
                segments: allSegments,
                metadata: (_a = processedBatches[0]) === null || _a === void 0 ? void 0 : _a.metadata // Use metadata from first batch
            },
            usage: {
     ...
     */
    async processSingleBatch(request, config, processingType) {
        ...
        while (retries <= maxRetries) {
            try {
                ...
                const { content, usage } = await this.callOpenAI(systemPrompt, userPrompt, { responseFormat: processingType === 'shadowing' ? 'text' : 'json' });
                ...
                if (!processed.segments || processed.segments.length === 0) {
                    throw new types_1.AIServiceError('No valid segments generated', 'NO_SEGMENTS', 500);
                }
        ...
        // All retries failed
        throw new types_1.AIServiceError(`Failed to process transcript after ${maxRetries + 1} attempts: ${(lastError === null || lastError === void 0 ? void 0 : lastError.message) || 'Unknown error'}`, 'PROCESSING_FAILED', 500, { originalError: lastError });
    }
    /**
     * Validate request
     */
    validateRequest(request) {
        ...
        if (!request.content.transcript || !Array.isArray(request.content.transcript)) {
            throw new types_1.AIServiceError('Transcript must be an array of segments', 'VALIDATION_ERROR', 400);
        }
        if (request.content.transcript.length === 0) {
            throw new types_1.AIServiceError('Transcript cannot be empty', 'VALIDATION_ERROR', 400);
        }
        if (request.maxSegmentLength && (request.maxSegmentLength < 5 || request.maxSegmentLength > 100)) {
            throw new types_1.AIServiceError('Max segment length must be between 5 and 100', 'VALIDATION_ERROR', 400);
        }
    }
    /**
     * Determine processing type based on request flags
     */
    determineProcessingType(request) {
     ...
     */
    getSystemPrompt(config, processingType) {
...
Return processed segments in JSON format${includeTranslations ? ' and include the "translation" field where applicable.' : '.'}`;
     ...
     */
    getUserPrompt(request, config, processingType) {
...
JLPT Level: ${(config === null || config === void 0 ? void 0 : config.jlptLevel) || 'N5'}
...
Return processed segments with vocabulary in JSON format${includeTranslations ? ' and include each translation in a "translation" field.' : '.'}`;
     ...
     */
    parseTranscriptResponse(response, processingType) {
        let parsed;
        try {
            ...
            throw new types_1.AIServiceError('Failed to parse AI response as JSON', 'PARSE_ERROR', 500, { response: response.substring(0, 500) });
        }
        ...
        if (processingType === 'shadowing') {
            ...
            if (Array.isArray(parsed)) {
                return {
                    ...
                    }).filter(seg => seg !== null), // Remove null segments
                    vocabulary: []
                };
            }
                ...
                throw new types_1.AIServiceError('Invalid response format for shadowing', 'INVALID_FORMAT', 500, { parsed });
            }
        }
        ...
        if (Array.isArray(segments)) {
            const validSegments = segments.map((seg, index) => {
                if (typeof seg === 'string') {
                    ...
                    return Object.assign(Object.assign({}, seg), { id: seg.id || `seg_${index + 1}` });
                }
                return null;
            }).filter(seg => seg !== null);
            return {
     ...
     * Enhance processed transcript
     */
    enhanceTranscript(transcript, request, config) {
        ...
        if (request.content.transcript.length > 0 && (!transcript.vocabulary || transcript.vocabulary.length === 0)) {
            transcript.vocabulary = this.extractVocabulary(transcript.segments, config === null || config === void 0 ? void 0 : config.jlptLevel);
        }
     ...
     */
    extractVocabulary(segments, jlptLevel) {
        const vocabulary = [];
        const seen = new Set();
     ...
     */
    async formatForShadowing(transcript, maxSegmentLength = 20, config) {
        return this.process({
            content: {
                transcript,
                language: 'ja'
     ...
     */
    async fixTranscriptErrors(transcript, videoTitle, config) {
        return this.process({
            content: {
                transcript,
                videoTitle,
...
exports.TranscriptProcessor = TranscriptProcessor;
//# sourceMappingURL=TranscriptProcessor.js.map