...
exports.GrammarExplainerProcessor = void 0;
const BaseProcessor_1 = require("./BaseProcessor");
const types_1 = require("../types");
class GrammarExplainerProcessor extends BaseProcessor_1.BaseProcessor {
    constructor(context) {
        super(context);
    }
    /**
    ...
    validateRequest(request) {
        if (!request.content) {
            throw new types_1.AIServiceError('Content is required for grammar explanation', 'VALIDATION_ERROR', 400);
        }
        if (typeof request.content !== 'string' || request.content.trim().length === 0) {
            throw new types_1.AIServiceError('Content must be a non-empty string', 'VALIDATION_ERROR', 400);
        }
        // Check content length
        if (request.content.length > 1000) {
            throw new types_1.AIServiceError('Content too long. Maximum 1000 characters.', 'VALIDATION_ERROR', 400);
        }
    }
    /**
...
Return your response as a valid json object as specified.`;
    ...
    parseResponse(response) {
        ...
        if (!parsed.pattern || !parsed.meaning || !parsed.structure) {
            throw new types_1.AIServiceError('Missing required fields in grammar explanation', 'INVALID_RESPONSE_FORMAT', 500);
        }
     ...
     */
    generateCommonMistakes(pattern) {
        const mistakes = [];
     ...
     */
    async explainWithComparison(pattern, comparePatterns, config) {
        return this.process({
            content: pattern,
     ...
     */
    async explainFromSentence(sentence, config) {
        return this.process({
            content: sentence,
...
exports.GrammarExplainerProcessor = GrammarExplainerProcessor;
//# sourceMappingURL=GrammarExplainerProcessor.js.map