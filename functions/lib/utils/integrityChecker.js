...
exports.checkNewsArticles = checkNewsArticles;
exports.checkStories = checkStories;
exports.checkBooks = checkBooks;
exports.runIntegrityCheck = runIntegrityCheck;
const admin = __importStar(require("firebase-admin"));
const logger = __importStar(require("firebase-functions/logger"));
const newsAudioGenerator_1 = require("./newsAudioGenerator");
const translationPreGenerator_1 = require("./translationPreGenerator");
const wordExplanationPreGenerator_1 = require("./wordExplanationPreGenerator");
const wordExtractor_1 = require("./wordExtractor");
const integrityIdempotency_1 = require("./integrityIdempotency");
 ...
 */
async function checkNewsArticles(checkId) {
    const result = {
        checked: 0,
        missingAudio: [],
        missingTranslations: [],
    ...
    try {
        ...
        const wordExplanationIds = new Set();
        for (let i = 0; i < articleIds.length; i += 30) {
            const batch = articleIds.slice(i, i + 30);
 ...
 */
function selectArticlesForRepair(result, maxArticles) {
    const articlesToRepair = new Set();
    ...
    for (const [articleId] of sorted) {
        if (articlesToRepair.size >= maxArticles)
            break;
 ...
 */
async function repairNewsArticles(articleIds, result, checkId) {
    ...
    const articles = articlesSnapshot.docs.map(doc => ({
        ...
        nhkAudioUrl: doc.data().nhkAudioUrl,
    }));
    // Repair audio for articles that need it
    const articlesNeedingAudio = articles.filter(a => result.missingAudio.includes(a.id) || result.failedAudio.includes(a.id));
    if (articlesNeedingAudio.length > 0 && checkId) {
        await (0, integrityIdempotency_1.updateCheckProgress)(checkId, 'repairing_article_audio', {
            total: articlesNeedingAudio.length,
        });
    }
    for (const article of articlesNeedingAudio) {
        // Check cooldown before attempting repair
        const canRepair = await (0, integrityIdempotency_1.canRepairContent)(article.id, 'news_article', 'audio');
        ...
        try {
            ...
            if (checkId) {
                await (0, integrityIdempotency_1.recordRepairAttempt)(article.id, 'news_article', 'audio', checkId, true);
            }
        }
        catch (error) {
            ...
            if (checkId) {
                await (0, integrityIdempotency_1.recordRepairAttempt)(article.id, 'news_article', 'audio', checkId, false, errorMsg);
            }
    ...
    const articlesNeedingTranslations = articles.filter(a => result.missingTranslations.includes(a.id));
    // Filter out articles in cooldown for translations
    const articlesForTranslationRepair = [];
    for (const article of articlesNeedingTranslations) {
        const canRepair = await (0, integrityIdempotency_1.canRepairContent)(article.id, 'news_article', 'translations');
        if (canRepair.allowed) {
    ...
    if (articlesForTranslationRepair.length > 0) {
        if (checkId) {
            await (0, integrityIdempotency_1.updateCheckProgress)(checkId, 'repairing_translations', {
        ...
        try {
            ...
            const translationResults = await (0, translationPreGenerator_1.generateBatchTranslations)(articlesForTranslationRepair);
            ...
            if (checkId) {
                for (const article of articlesForTranslationRepair) {
                    ...
                    await (0, integrityIdempotency_1.recordRepairAttempt)(article.id, 'news_article', 'translations', checkId, success, success ? undefined : 'Batch translation failed');
                }
            }
        }
        catch (error) {
            ...
            if (checkId) {
                for (const article of articlesForTranslationRepair) {
                    await (0, integrityIdempotency_1.recordRepairAttempt)(article.id, 'news_article', 'translations', checkId, false, errorMsg);
                }
            }
    ...
    const articlesNeedingWordExplanations = articles.filter(a => result.missingWordExplanations.includes(a.id));
    // Filter out articles in cooldown for word explanations
    const articlesForWordRepair = [];
    for (const article of articlesNeedingWordExplanations) {
        const canRepair = await (0, integrityIdempotency_1.canRepairContent)(article.id, 'news_article', 'wordExplanations');
        if (canRepair.allowed) {
    ...
    if (articlesForWordRepair.length > 0) {
        if (checkId) {
            await (0, integrityIdempotency_1.updateCheckProgress)(checkId, 'repairing_word_explanations', {
        ...
        try {
            ...
            if (checkId) {
                for (const article of articlesForWordRepair) {
                    ...
                    await (0, integrityIdempotency_1.recordRepairAttempt)(article.id, 'news_article', 'wordExplanations', checkId, success, success ? undefined : 'Batch word explanation failed');
                }
            }
        }
        catch (error) {
            ...
            if (checkId) {
                for (const article of articlesForWordRepair) {
                    await (0, integrityIdempotency_1.recordRepairAttempt)(article.id, 'news_article', 'wordExplanations', checkId, false, errorMsg);
                }
            }
 ...
 */
async function checkStories(adminKey, checkId, maxStories = CONFIG.MAX_STORIES_PER_RUN) {
    const result = {
        checked: 0,
        missingAudio: [],
        missingImages: [],
    ...
    try {
        ...
        for (const doc of storiesSnapshot.docs) {
            ...
            const pagesWithMissingTranslations = pages.filter((p) => p.text && !p.translation && !p.textEn).length;
 ...
 */
async function repairStory(storyId, result, adminKey, checkId) {
    var _a, _b, _c, _d;
    ...
    try {
        ...
        if (result.missingAudio.includes(storyId)) {
            // Check cooldown before attempting repair
            const canRepair = await (0, integrityIdempotency_1.canRepairContent)(storyId, 'story', 'audio');
            if (!canRepair.allowed) {
                ...
                // Update progress
                if (checkId) {
                    await (0, integrityIdempotency_1.updateCheckProgress)(checkId, 'repairing_story_audio', { stage: storyId });
                }
                try {
                    ...
                    if (pages.length === 0) {
                        ...
                        result.repairFailed.audio++;
                        if (checkId) {
                            await (0, integrityIdempotency_1.recordRepairAttempt)(storyId, 'story', 'audio', checkId, false, 'No pages');
                        }
                        return;
                    }
                    ...
                    if (response.ok) {
                        ...
                        if (checkId) {
                            await (0, integrityIdempotency_1.recordRepairAttempt)(storyId, 'story', 'audio', checkId, true);
                        }
                    }
                    else {
                        result.repairFailed.audio++;
                        ...
                        if (checkId) {
                            await (0, integrityIdempotency_1.recordRepairAttempt)(storyId, 'story', 'audio', checkId, false, `HTTP ${response.status}: ${errorText.substring(0, 200)}`);
                        }
                    ...
                    const errorMsg = error instanceof Error ? error.message : 'Unknown error';
                    if (checkId) {
                        await (0, integrityIdempotency_1.recordRepairAttempt)(storyId, 'story', 'audio', checkId, false, errorMsg);
                    }
        ...
        if (result.missingImages.includes(storyId)) {
            // Check cooldown before attempting repair
            const canRepair = await (0, integrityIdempotency_1.canRepairContent)(storyId, 'story', 'images');
            if (!canRepair.allowed) {
                ...
                if (!storyData.characterSheet) {
                    ...
                    // Update progress
                    if (checkId) {
                        const missingPages = ((_a = storyData.pages) === null || _a === void 0 ? void 0 : _a.filter((p) => !p.imageUrl).length) || 0;
                        await (0, integrityIdempotency_1.updateCheckProgress)(checkId, 'repairing_story_images', {
                            stage: storyId,
                            total: missingPages,
                        });
                    }
                    try {
                        logger.info('[IntegrityChecker] Repairing story images', {
                            storyId,
                            checkId,
                            missingPages: (_b = storyData.pages) === null || _b === void 0 ? void 0 : _b.filter((p) => !p.imageUrl).length,
                            totalPages: (_c = storyData.pages) === null || _c === void 0 ? void 0 : _c.length,
                        });
                        ...
                        if (response.ok) {
                            ...
                            result.repaired.images = ((_d = repairResult.pagesRepaired) === null || _d === void 0 ? void 0 : _d.length) || 0;
                            ...
                            if (checkId) {
                                await (0, integrityIdempotency_1.recordRepairAttempt)(storyId, 'story', 'images', checkId, true);
                            }
                        }
                        else {
                            ...
                            const errorText = await response.text();
                            if (checkId) {
                                await (0, integrityIdempotency_1.recordRepairAttempt)(storyId, 'story', 'images', checkId, false, `HTTP ${response.status}: ${errorText.substring(0, 200)}`);
                            }
                        ...
                        const errorMsg = error instanceof Error ? error.message : 'Unknown error';
                        if (checkId) {
                            await (0, integrityIdempotency_1.recordRepairAttempt)(storyId, 'story', 'images', checkId, false, errorMsg);
                        }
 ...
 */
async function checkBooks(checkId) {
    const result = {
        checked: 0,
        missingTranslations: [],
    };
...
async function runIntegrityCheck(adminKey, checkId, options) {
    const startTime = Date.now();
    // Allow overriding max items per run (clamped to 1-5 for safety)
    const maxStories = Math.min(5, Math.max(1, (options === null || options === void 0 ? void 0 : options.maxStories) || CONFIG.MAX_STORIES_PER_RUN));
    const maxArticles = Math.min(5, Math.max(1, (options === null || options === void 0 ? void 0 : options.maxArticles) || CONFIG.MAX_ARTICLES_PER_RUN));
    logger.info('[IntegrityChecker] Starting full integrity check', {
        checkId,
        maxArticles,
        maxStories,
        lookbackDays: CONFIG.LOOKBACK_DAYS,
        isOverride: !!((options === null || options === void 0 ? void 0 : options.maxStories) || (options === null || options === void 0 ? void 0 : options.maxArticles)),
    });
    // Update progress: checking articles
    if (checkId) {
        await (0, integrityIdempotency_1.updateCheckProgress)(checkId, 'checking_articles');
    }
    const newsResult = await checkNewsArticles(checkId);
    // Update progress: checking stories
    if (checkId) {
        await (0, integrityIdempotency_1.updateCheckProgress)(checkId, 'checking_stories');
    }
    const storiesResult = await checkStories(adminKey, checkId, maxStories);
    // Update progress: checking books
    if (checkId) {
        await (0, integrityIdempotency_1.updateCheckProgress)(checkId, 'checking_books');
    }
    ...
    logger.info('[IntegrityChecker] Full integrity check complete', {
        ...
        newsRepaired: newsResult.repaired.audio +
