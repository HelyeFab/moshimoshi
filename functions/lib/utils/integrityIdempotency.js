 ...
 */
function generateCheckId(type, scheduleTime, requestId) {
...
async function markCheckStarted(checkId, type, triggeredBy, scheduleTime) {
  const ref = db.collection('ops').doc('integrity').collection('processed_checks').doc(checkId)
  const ttl = firestore_1.Timestamp.fromMillis(
    Date.now() + exports.IDEMPOTENCY_CONFIG.CHECK_TTL_DAYS * 24 * 60 * 60 * 1000
  )
  const data = {
    checkId,
    type,
...
async function recordRepairAttempt(contentId, contentType, repairType, checkId, success, error) {
  ...
  const ttl = firestore_1.Timestamp.fromMillis(
    Date.now() + exports.IDEMPOTENCY_CONFIG.REPAIR_TTL_DAYS * 24 * 60 * 60 * 1000
  )
  const attemptRecord = {
    attemptedAt: now,
    checkId,
  ...
  await db.runTransaction(async transaction => {
    ...
    if (!snap.exists) {
      ...
      if (!success && consecutiveFailures >= 2) {
        ...
        const cooldownHours =
          exports.IDEMPOTENCY_CONFIG.REPAIR_COOLDOWN_HOURS * Math.pow(2, consecutiveFailures - 1)
        ...
        cooldownUntil = firestore_1.Timestamp.fromMillis(
          Date.now() + actualCooldownHours * 60 * 60 * 1000
        )
      }
...
async function tryAcquireLock(instanceId) {
  ...
  const now = Date.now()
  const expiresAt = firestore_1.Timestamp.fromMillis(
    now + exports.IDEMPOTENCY_CONFIG.LOCK_EXPIRY_MINUTES * 60 * 1000
  )
  try {
    await db.runTransaction(async transaction => {
      const snap = await transaction.get(ref)
...
async function updateLockHeartbeat(instanceId) {
  const ref = db.collection('ops').doc('integrity').collection('locks').doc('integrity_checker')
  const expiresAt = firestore_1.Timestamp.fromMillis(
    Date.now() + exports.IDEMPOTENCY_CONFIG.LOCK_EXPIRY_MINUTES * 60 * 1000
  )
  try {
    await ref.update({
